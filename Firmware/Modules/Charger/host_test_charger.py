...
def test_setpoints_and_timing():
    ...
    check(re.search(r"#define CHG_ABSORB_TIMED_MAX_MV\s+14500u", text_h), "soak must count only up to 14.5 V")
    ...
    check(re.search(r"#define FAULT_BAT_DISCONNECT_DEBOUNCE_MS\s+100u", text_fault_h), "pump debounce must be 100 ms = 10 control passes (user directive; the pumped node holds >14.8 V ~0.5 s after the 15.0 V cut so it still latches)")
