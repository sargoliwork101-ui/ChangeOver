...
void func__Charger_Evaluate(const measurement_snapshot_t *measurement_snapshot_t__snap, app_state_t app_state_t__state)
{
    ...
    /* [EN] Battery missing — check V_BAT_LOW (MID-GND) and V_BAT_HIGH (V24-MID) separately.
       Trans2 (CH2, 0x02) only controls V_BAT_HIGH battery (MID-V24) via PA6/PA7/PB6.
       For this test (Trans2 only), V_BAT_HIGH <2V → battery missing for CH2.
       V_BAT_LOW is monitored for balance but not for CH2 charging.
       [FA] باتری غایب جداگانه برای هر کانال. */
    {
        uint32_t uint32_t__vBatLowChk = uint32_t__vbat12;
        uint32_t uint32_t__vBatHighChk = 0u;
        if (uint32_t__vbat24 > uint32_t__vbat12) { uint32_t__vBatHighChk = uint32_t__vbat24 - uint32_t__vbat12; }
        bool bool__lowMissing = (uint32_t__vBatLowChk < CHG_BAT_12V_MISSING_MV);
        bool bool__highMissing = (uint32_t__vBatHighChk < CHG_BAT_12V_MISSING_MV);
        /* [EN] Trans2 only: check high battery. If high missing → fault.
           Low battery missing is still monitored for balance but not charging Trans2.
           [FA] برای Trans2 فقط باتری بالا چک. */
        if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_2_MASK) != 0u)
        {
            if (bool__highMissing)
            {
                func__Charger_SafeOff();
        ...
        if (bool__lowMissing && bool__highMissing)
        {
            func__Charger_SafeOff();
            FAULT_MASK__G__Fault |= CHG_FAULT_BATTERY_MISSING;
            CHARGER_STATE__G__State = CHG_STATE_IDLE;
    ...
    /* [EN] JIT fault latched → stay FAULT. Do NOT return early here; let switch(FAULT) handle lockout/retry.
       If JIT in any channel, both PWM zero even if only one transfo (mask 0x02).
       The PWM zero + NC open is done in switch(FAULT) via OpenTransformerInput().
       This early return was removed to allow retry half/10% to actually execute (item 7).
       [FA] بدون return زودهنگام — retry در case FAULT اجرا شود. */
    /* [EN] No early return; fall through to Balance/Overcurrent and then switch. */
       ...
       [FA] جریان فیلترشده متوسط ورودی را با تخمین خروجی مقایسه کن، نه مستقیم 675. */
    {
        ...
        /* [EN] Per-channel: only check installed channel. Trans2 only → check Ch2.
           [FA] فقط کانال نصب‌شده. */
        bool bool__overAny = false;
        if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_1_MASK) != 0u) { bool__overAny = bool__overAny || bool__overCh1; }
        if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_2_MASK) != 0u) { bool__overAny = bool__overAny || bool__overCh2; }
        ...
        if (bool__overAny)
        {
            if (DUTY_PERMILLE__G__Current > 10u)
            {
                ...
                func__Charger_SetPwmChannel(CHG_CHANNEL_2_MASK, uint16_t__newDuty);
            }
            else
            {
    ...
    switch (CHARGER_STATE__G__State)
    {
        ...
        case CHG_STATE_PRECHARGE:
        {
            ...
            if ((uint32_t__nowMs - TICK__G__StateEnterMs) >= 1000u)
            {
                ...
                func__Charger_SetPwmChannel(CHG_CHANNEL_2_MASK, CHG_SOFT_START_DUTY_PERMILLE);
            }
            break;
        }
        ...
        case CHG_STATE_SOFTSTART:
        {
            ...
            if ((uint32_t__nowMs - TICK__G__LastRampMs) >= 1000u)
            {
                ...
                {
                    func__Charger_SetPwmChannel(CHG_CHANNEL_2_MASK, (uint16_t)uint32_t__newDuty);
                }
        ...
        case CHG_STATE_BULK:
        {
            ...
            /* [EN] Per-channel: Trans2 (CH2) only controls V_BAT_HIGH (MID-V24) at 14400.
               Trans1 disabled (mask 0x02) → V_BAT_LOW not used for CH2 transition.
               [FA] هر کانال ولتاژ خودش. */
            bool bool__trans2Absorb = false;
            uint32_t uint32_t__vBatHighForBulk = (uint32_t__vbat24 > uint32_t__vbat12) ? (uint32_t__vbat24 - uint32_t__vbat12) : 0u;
            if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_2_MASK) != 0u)
            {
                bool__trans2Absorb = (uint32_t__vBatHighForBulk >= CHG_12V_ABSORB_PER_CHANNEL_MV);
            }
            if (bool__trans2Absorb)
            {
               ...
               [FA] حلقه جریان Bulk واقعی. */
            {
                ...
                if ((uint32_t__iout1 < (uint32_t__target - 20u)) && (uint32_t__iout2 < (uint32_t__target - 20u)))
                {
                    if (DUTY_PERMILLE__G__Current < 1000u)
                    {
                        ...
                        func__Charger_SetPwmChannel(CHG_CHANNEL_2_MASK, uint16_t__next);
                    }
                }
        ...
        case CHG_STATE_ABSORB:
        {
            ...
            /* [EN] Per-channel tail: Trans2 (CH2) only checks Ch2 tail and V_BAT_HIGH (MID-V24) at 14400.
               Ch1 disabled → ignore Ch1 tail/V_BAT_LOW.
               [FA] هر کانال Tail جدا. */
            ...
            uint32_t uint32_t__vBatHighAbs = (uint32_t__vbat24 > uint32_t__vbat12) ? (uint32_t__vbat24 - uint32_t__vbat12) : 0u;

            if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_2_MASK) != 0u)
            {
                if ((uint32_t__vBatHighAbs >= (CHG_12V_ABSORB_PER_CHANNEL_MV - 100u)) &&
                    (uint32_t__vBatHighAbs <= (CHG_12V_ABSORB_PER_CHANNEL_MV + 100u)))
                {
            ...
            if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_1_MASK) != 0u)
            {
                if ((uint32_t__vbat12 >= (CHG_12V_ABSORB_PER_CHANNEL_MV - 100u)) &&
                    (uint32_t__vbat12 <= (CHG_12V_ABSORB_PER_CHANNEL_MV + 100u)))
                {
                    bool__voltageInAbsorb = true;
                }
            }

            bool bool__tailOk = false;
            if ((CHG_INSTALLED_CHANNEL_MASK == CHG_CHANNEL_2_MASK)) { bool__tailOk = bool__tailLowCh2; }
            else if ((CHG_INSTALLED_CHANNEL_MASK == CHG_CHANNEL_1_MASK)) { bool__tailOk = bool__tailLowCh1; }
            else { bool__tailOk = bool__tailLowCh1 && bool__tailLowCh2; }

            if (bool__tailOk && bool__voltageInAbsorb)
            {
            ...
            /* [EN] Real Absorb CV loop per-channel: Trans2 only regulates V_BAT_HIGH at 14400.
               [FA] حلقه ولتاژ Absorb هر کانال. */
            {
                uint32_t uint32_t__targetHigh = CHG_12V_ABSORB_PER_CHANNEL_MV;
                uint32_t uint32_t__vBatHighLoop = (uint32_t__vbat24 > uint32_t__vbat12) ? (uint32_t__vbat24 - uint32_t__vbat12) : 0u;
                bool bool__low = false;
                bool bool__high = false;
                if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_2_MASK) != 0u)
                {
                    bool__low = (uint32_t__vBatHighLoop < (uint32_t__targetHigh - 100u));
                    bool__high = (uint32_t__vBatHighLoop > (uint32_t__targetHigh + 100u));
                }
                if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_1_MASK) != 0u)
                {
                    bool__low = bool__low || (uint32_t__vbat12 < (CHG_12V_ABSORB_PER_CHANNEL_MV - 100u));
                    bool__high = bool__high || (uint32_t__vbat12 > (CHG_12V_ABSORB_PER_CHANNEL_MV + 100u));
                }
        ...
        case CHG_STATE_FLOAT:
        {
            ...
            /* [EN] Per-channel Float reentry: Trans2 only checks V_BAT_HIGH.
               [FA] هر کانال Float جدا. */
            uint32_t uint32_t__vBatHighFloat = (uint32_t__vbat24 > uint32_t__vbat12) ? (uint32_t__vbat24 - uint32_t__vbat12) : 0u;
            bool bool__reentryHigh = (uint32_t__vBatHighFloat < CHG_12V_FLOAT_PER_CHANNEL_MV - 700u); /* 13500-700=12800 */
            bool bool__reentryLow = (uint32_t__vbat12 < CHG_12V_REENTRY_MV);
            bool bool__reentry = false;
            if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_2_MASK) != 0u) { bool__reentry = bool__reentryHigh; }
            if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_1_MASK) != 0u) { bool__reentry = bool__reentry || bool__reentryLow; }
            if ((CHG_INSTALLED_CHANNEL_MASK == CHG_CHANNEL_BOTH_MASK)) { bool__reentry = bool__reentryHigh || bool__reentryLow; }
            ...
            /* [EN] Real Float CV loop per-channel: Trans2 only regulates V_BAT_HIGH at 13500.
               [FA] حلقه ولتاژ Float هر کانال. */
            {
                uint32_t uint32_t__targetHighF = CHG_12V_FLOAT_PER_CHANNEL_MV;
                uint32_t uint32_t__vBatHighF = uint32_t__vBatHighFloat;
                bool bool__lowF = false;
                bool bool__highF = false;
                if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_2_MASK) != 0u)
                {
                    bool__lowF = (uint32_t__vBatHighF < (uint32_t__targetHighF - 100u));
                    bool__highF = (uint32_t__vBatHighF > (uint32_t__targetHighF + 100u));
                }
                if ((CHG_INSTALLED_CHANNEL_MASK & CHG_CHANNEL_1_MASK) != 0u)
                {
                    bool__lowF = bool__lowF || (uint32_t__vbat12 < (CHG_12V_FLOAT_PER_CHANNEL_MV - 100u));
                    bool__highF = bool__highF || (uint32_t__vbat12 > (CHG_12V_FLOAT_PER_CHANNEL_MV + 100u));
                }
        ...
        case CHG_STATE_FAULT:
        {
            ...
            if ((FAULT_MASK__G__Fault & (CHG_FAULT_JIT1 | CHG_FAULT_JIT2)) != 0u)
            {
                ...
                if (JIT_RETRY_STEP__G__Count == 0u)
                {
                    ...
                    TICK__G__RelayCloseMs = 0u;
                    CHARGER_STATE__G__State = CHG_STATE_PRECHECK;
                    func__Charger_SetPwmChannel(CHG_CHANNEL_2_MASK, uint16_t__retryDuty);
                    break;
                }
