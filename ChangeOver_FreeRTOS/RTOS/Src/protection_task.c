/**
 * @file RTOS/Src/protection_task.c
 * @brief C source for protection_task.
 ...
 * @function protection_task_entry
 ...
 */
static void protection_task_entry(void * context)
{
    protection_task_t * task;
    TickType_t last_wake;
    measurement_snapshot_t measurement;
    fault_mask_t faults;

    task = (protection_task_t *)context;
    if ((task != NULL) && (task->measurements != NULL) &&
        (task->faults != NULL) && (task->diagnostics != NULL))
    {
        last_wake = xTaskGetTickCount();
        for (;;)
        {
            measurement = measurement_manager_get_snapshot(task->measurements);
            faults = battery_protection_evaluate(&measurement);
            fault_manager_replace_dynamic(task->faults, faults);
            if ((faults & FAULT_LOW_BATTERY) != FAULT_NONE)
            {
            ...
            if (measurement.current2_ma > APP_CONFIG.over_current_2_ma)
            {
                diagnostics_report(task->diagnostics,
                                   DIAG_OVERCURRENT_CH2,
                                   DIAG_SEVERITY_ERROR,
            ...
            vTaskDelayUntil(&last_wake,
                            pdMS_TO_TICKS(APP_CONFIG.protection_period_ms));
        }
    }
    vTaskDelete(NULL);
}
                            ...
                            diagnostics_t * diagnostics)
{
    ...
    if ((task != NULL) && (measurements != NULL) &&
        (faults != NULL) && (diagnostics != NULL))
    {
        task->measurements = measurements;
        task->faults = faults;
        task->diagnostics = diagnostics;
        task->handle = NULL;
        task->handle = xTaskCreateStatic(protection_task_entry,
                                         "protection",
                                         PROTECTION_TASK_STACK_WORDS,
                                         task,
                                         PROTECTION_TASK_PRIORITY,
                                         task->stack,
                                         &task->control_block);
