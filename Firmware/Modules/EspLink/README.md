...
پروتکل فرمان و تله‌متری پیاده شده (`esp_link.c`): پارسر فریم باینری، اعمال پارامترها، تله‌متری دوره‌ای. `MODULE_ESP = 0` مانده تا کاربر فعالش کند (چک قوانین فعلاً MODULE_ESP را 0 می‌خواهد). با فعال‌شدن، `EspLink_Init` در `TaskComm` UART را با RX وقفه‌ای بالا می‌آورد و CH_PD را High می‌کند. سند کامل پروتکل برای سمت ESP: `ESP_AGENT_SPEC.md` در ریشهٔ repo.

## تاریخچه

| تاریخ | تغییر |
|---|---|
| 2026-09-22 | اجرای AI (دستور کاربر): پروتکل کامل — SET_PARAM/GET_PARAMS/TLM_LIVE/PARAM_REPORT/PARAMS_BULK، ۱۴ پارامتر زنده (آفست/گین جریان، آفست ولتاژ، فیلترها، بازدهی، قطع/وصل کانال)؛ RX وقفه‌ای ۱۲۸ بایتی در bsp_uart |
