...
def run_tests():
    ...
    # 41 installed channel mask 0x02 Trans2 only (PWM2 PA6)
    assert_equal(d("CHG_INSTALLED_CHANNEL_MASK",0x02),0x02,"41 mask 0x02 Trans2 only")
    ...
    assert_true((d("CHG_INSTALLED_CHANNEL_MASK",0x02) & 0x02)!=0, "41 ch2 enabled Trans2")
    assert_true((d("CHG_INSTALLED_CHANNEL_MASK",0x02) & 0x01)==0, "41 ch1 disabled Trans2")
    ...
    # Check no SetPwmBoth call (only Channel 2) in active code (definition allowed)
    assert_true(c_content.count("func__Charger_SetPwmBoth(") == 1, "42 no SetPwmBoth call, only def")
    assert_true("func__Charger_SetPwmChannel(CHG_CHANNEL_2_MASK" in c_content, "42 per-channel CH2")
    ...
    assert_true(d("CHG_24V_ABSORB_MV",28800)==28800, "43 24V still 28800 but not used as single 24V charger")
    assert_true("BSP_PWM_CHARGER_2" in c_content and "PA6" in open(os.path.join(BASE_DIR,"..","..","..","Firmware","Config","Inc","board_pins.h")).read(), "43 Trans2 PWM2 PA6")
