 ...
 *            Why only 100 ms (= 10 control passes, user directives
 ...
 *            14.8 V for ~0.5 s, so 10 consecutive passes still latch long
 *            before it decays away - and 50 ms proved too thin on the bench
 *            (spontaneous over-band hops), so the user raised it to 100 ms.
 ...
 *         حالت با یک پرچم خطا پوشش داده می‌شوند - بالای ۱۴٫۸V به‌مدت ۱۰۰ms
 *         (۱۰ پاس پشت‌سر؛ امضای پمپ حین شارژ - چون قطع‌سخت ۱۵٫۰V پمپ را در
 *         حد میلی‌ثانیه می‌خواباند، دبانس ۳۰۰ms قدیمی هیچ‌وقت پر نمی‌شد، و
 *         گره پمپ‌شده ~۰٫۵s بالای ۱۴٫۸V شناور می‌ماند پس ۱۰۰ms هم فراوان
 *         می‌رسد؛ دستور کاربر پس از پرش‌های خودبه‌خود ۵۰ms)
...
#define FAULT_BAT_DISCONNECT_DEBOUNCE_MS    100u
