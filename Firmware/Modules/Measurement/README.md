...
- **جریان شارژ از زنجیرهٔ فیلتر کلیددار عبور می‌کند**: پورت برد دو جایگاه CURRENT1/CURRENT2 فریم را با نمونهٔ خامِ سنکرون با PWM پر می‌کند (تبدیل سخت‌افزاری ADC2 با تریگر لبهٔ وسط پنجرهٔ ON گیت — `TIM2_CC2` و `TIM3_TRGO`)؛ این ماژول نمونه را به mA تبدیل و بعد از **مدین-۳** (حذف پرش تک‌نمونه‌ای) و **میانگین متحرک ۱۰ نمونه** (صاف‌کردن، هر کانال پنجرهٔ جدا) عبور می‌دهد. هر دو با ثابت کامپایل `MEASUREMENT_CURRENT_MEDIAN3_ENABLE` / `MEASUREMENT_CURRENT_AVERAGE_ENABLE` در `measurement.h` جداگانه روشن/خاموش می‌شوند (۱u=فعال، ۰u=کامپایل‌نشده؛ طول پنجره هم `MEASUREMENT_CURRENT_AVERAGE_WINDOW`، پیش‌فرض هر دو روشن). ترتیب ثابت است: اول مدین، بعد میانگین. اگر نمونهٔ سنکرون ممکن نشود (گیت پارک/timeout)، مقدار اسکن غیرهمزمان همان جایگاه می‌ماند و از همان زنجیره رد می‌شود.
- فرمول تبدیل ADC→mA در `bsp_measurement` مرحله‌به‌مرحله از مدار ساخته شده (شمارش→mV پایه → خنثی‌سازی تقسیم R41/R42 → خنثی‌سازی گین ۱۰۱ → شانت 10mΩ → گین پرمیل بنچ)؛ جزئیات در README ماژول Bsp.
- ولتاژها همچنان مدین-۵ ضد spike دارند (دست‌نخورده).

## تاریخچه

| تاریخ | تغییر |
|---|---|
| 2026-09-19 | **مدین-۵ روی ولتاژ دو نیم‌باتری** (`v_bat_low`/`v_bat_high`؛ ارتقای همان‌روز از مدین-۳): بوق فیک حین ابزوربِ واقعی نشان داد برست‌های دو-فریمی از مدین-۳ رد می‌شوند؛ مدین-۵ هر ترکیدگی کوتاه‌تر از ۳ فریم را می‌کشد و پلهٔ واقعی را با ~۲۰ms تأخیر رد می‌کند |
| 2026-09-19 | مدین-۳ روی ولتاژ دو نیم‌باتری (دستور کاربر): اسپایک‌های تک‌فریمی ADC روی گره سوئیچینگ، آستانهٔ ۱۴٫۸V آلارم قطع باتری را الکی می‌شکستند (جایگزینِ همان‌روزش: مدین-۵) |
...
| `func__Measurement_Run` | یک فریم normalized را از BSP می‌گیرد (جریان‌ها = نمونهٔ سنکرون وسط ON از پورت برد)، به mV/mA تبدیل می‌کند؛ جریان هر کانال از زنجیرهٔ کلیددار مدین-۳ → میانگین متحرک ۱۰تایی رد می‌شود و ولتاژهای نیم‌باتری از مدین-۵؛ `input_present` را از سیگنال منطقی BSP می‌خواند؛ پس از سه فریم کامل و پایدار `valid = true` می‌شود |
...
حافظه: بافر DMA = 2×5×2 = 20 بایت استاتیک؛ استک تسک `TASK_STACK_MEASUREMENT` = 192 word (768 بایت) — بعد از Build، مصرف کل RAM/Flash را از Map file چک کنید (قانون مدیریت حافظه). بک‌اند سنکرون ADC2 در پورت برد حدود ۵۰ بایت RAM استاتیک اضافه می‌کند (هندل خصوصی `ADC_HANDLETYPEDEF__G__HadcSync` در `bsp_adc.c`)؛ بافرهای فیلتر جریان هم با هر دو کلید روشن حدود ۱۰۸ بایت RAM استاتیک اضافه می‌کنند (تاریخچهٔ مدین-۳ ۲×۳×4 + پنجرهٔ میانگین ۲×۱۰×4 + شمارنده‌ها) و با خاموش‌کردن هر کلید کاملاً از کامپایل خارج می‌شوند؛ مصرف دقیق بعد از Build واقعی CubeIDE از Map file ثبت شود.
