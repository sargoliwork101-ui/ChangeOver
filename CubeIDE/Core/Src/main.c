...
int main(void)
{
  ...
  MX_TIM3_Init();
  MX_USART1_UART_Init();
  /* [EN] Apply the second safe-state write after all GPIO modes exist.
     [FA] پس از ایجاد همهٔ modeهای GPIO، وضعیت امن یک بار دیگر اعمال می‌شود. */
  func__BspGpio_Init();
  func__BspPwm_Init();
  func__BspUart_Init();
  func__BspExti_Init();
...
static void MX_ADC1_Init(void)
{
  ...
  /** Configure for the selected regular conversion channels */
...
/**
  * @brief TIM2 PWM Initialization Function
  * @param None
  * @retval None
  */
static void MX_TIM2_Init(void)
...
/**
  * @brief TIM3 PWM Initialization Function
  * @param None
  * @retval None
  */
static void MX_TIM3_Init(void)
...
/**
  * @brief USART1 ESP-Link Initialization Function
  * @param None
  * @retval None
  */
static void MX_USART1_UART_Init(void)
...
static void MX_GPIO_Init(void)
{
  ...
  /* [EN] Set output latches before switching pins to output mode.
     [FA] پیش از تبدیل پایه‌ها به خروجی، latchهای خروجی را امن تنظیم می‌کنیم. */
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4 | GPIO_PIN_8, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(GPIOB,
                    GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_7 | GPIO_PIN_10,
                    GPIO_PIN_RESET);
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_11, GPIO_PIN_SET);
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_RESET);

  /* Active-high digital outputs: buzzer and ESP power. */
  ...
  /* Active-high LEDs/relay plus active-low battery controls. */
  ...
  /* Rising and falling EXTI inputs preserve comparator and presence events. */
