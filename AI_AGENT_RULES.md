...
- `ui_buzzer.c` فقط سرویس `func__Ui_Buzzer_Tick` را پیاده می‌کند؛ سناریوها از طریق همین API و بدون نوشتن مستقیم پایه یا نام برد، سیگنال منطقی `BSP_GPIO_BUZZER` را کنترل می‌کنند.
...
## جایگذاری ثابت‌های مرتبط با تابع

- هر ثابت پیکربندی که مستقیماً به یک تابع عمومی مربوط است، باید در همان فایل header و بلافاصله بالای prototype همان تابع تعریف شود؛ ثابت را در ابتدای یک بخش نامرتبط یا انتهای header پنهان نکن.
- بالای هر ثابت، توضیح دو‌زبانهٔ مهندسی ثبت شود و واحد، بازه و اثر ثابت روی رفتار تابع مشخص باشد.
- اگر ثابت فقط جزئیات خصوصی پیاده‌سازی است و قرارداد عمومی نیست، آن را در فایل `.c` و بلافاصله بالای همان تابع یا گروه تابع قرار بده؛ چنین ثابت خصوصی را بی‌دلیل وارد header نکن تا وابستگی و حجم preprocessing زیاد نشود.
- برای یک مفهوم فقط یک ثابت تعریف شود؛ از تکرار مقدار در چند header یا تعریف موازی در `.c` و `.h` خودداری کن.
- این قانون برای ثابت‌های جدید Agentها اجباری است و ثابت‌های موجود فقط هنگام تغییر همان بخش مرتب می‌شوند.
...
- مثال: `func__Ui_Init`, `func__Ui_BoardTest_Start`, `func__Ui_Buzzer_Tick`, `func__BspGpio_Write`, `func__App_Start`, `func__TaskUi`
...
5. **پایه‌ها/سیگنال‌ها** — اگر ماژول مستقیماً سخت‌افزار دارد، شناسهٔ منطقی BSP را مستند کن؛ نگاشت فیزیکی فعلی را فقط با برچسب «مرجع برد فعلی» بیاور و پایه را در منطق کد وارد نکن. اگر پایه/سیگنال ندارد، صریح بنویس.
6. **پیش‌فرض امن** — بعد از `Init` خروجی‌ها یا سیگنال‌های منطقی چه وضعی دارند
