...
`func__BspPwm_SetDutyPermille` دامنهٔ ۰ تا ۱۰۰۰ را اعمال می‌کند و صفر یعنی گیت پایین (compare=0). دو کانال PWM با **درهم‌گذاری فاز ۱۸۰ درجهٔ فریزشده** (نیم‌دوره = ۱۰µs در ۵۰kHz، دستور کاربر ۲۰۲۶-۰۹-۲۱: پالس کانال دو دقیقاً نیم‌دوره بعد از **استارت** پالس کانال یک) کار می‌کنند: در `func__BspPwm_Init` هر دو تایمر یک‌بار استارت می‌شوند — TIM2 از CNT=0 و TIM3 از CNT=ARR/2 (محاسبه از ARR واقعی، نه عدد ثابت) — و از آن‌به‌بعد **شمارنده‌ها هرگز متوقف یا بازنویسی نمی‌شوند**؛ خاموش‌کردن کانال فقط با compare=0 است تا آفست هیچ‌وقت نلغزد. چون هر دو روی یک کلاک ۷۲MHz و ARR یکسان‌اند رانش صفر است. `func__BspUart_Write` ارسال کامل با timeout محدود ۱۰۰ms دارد و `func__BspUart_ReadByte` non-blocking است.
